...
class Menu {
    ...
    /**
     * Given a keypath find a leaf node:
     * 
     * @example ```js
     * root.find('admin.users.create');
     * ```
     * 
     * @param {String} keypath Path to node
     * @param {Menu} [element=this] Node to start search from
     * @returns {Object} Node matching keypath. If no node found
     * we return undefined.
     */
    find(keypath, element=this) {
        if (element.keypath == keypath) return element;
        if (!element.nodes) return;

        let result;
        for(let node of element.nodes) {
            result = this.find(keypath, node);
     ...
     * `main.admin.debug.console`
     */
    get keypath() {
        if(this.parent) return this.parent.keypath + '.' + this.id;
        return this.id;
    }
